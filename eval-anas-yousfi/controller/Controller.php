...
class Controller
{
    ...
    public function handlerRequest() // permet de savoir ce que l'internaute demande (afficher/modifier/supprimer), action de l'internaute
    {
        ...
        try
        {
            if($op == 'add' || $op == 'update') $this->save($op); // si on ajoute ou modifie un , on appel la méthode save()
            elseif($op == 'select') $this->select(); // si on selectionne un , on appel la méthode select()
            elseif($op == 'delete') $this->delete(); // si on supprime un , on appel la méthode delete()
            else $this->selectAll(); // permettra d'afficher l'ensemble des s
        }
        catch(Exception $e)
        {
    ...
    public function selectAll()
    {
        ...
        $this->render('layout.php', 'donnees.php', array(
            ...
            'id' => 'id_' . ucfirst($this->db->table) // cela servira a pointé sur l'indice  du tableau de données envoyer dans le layout pour les liens voir/modifier/supprimer
        ));
    }
    ...
    public function select() // méthode permettant de voir le détail d'un , quand on clic sur le lien 'détail', c'est la méthode 'select()' qui s'execute
    {
        $id = isset($_GET['id']) ? $_GET['id'] : NULL;
        $this->render('layout.php', 'detail.php', array(
            "title" => "Détail de l' ID : $id",
    ...
    public function save($op)
    {
        ...
        $values = ($op == 'update') ? $this->db->select($id) : ''; // si on a envoyé un id dans l'URL, on l'envoi en argument de la méthode select() de entityRepository, cela permettra de selectionner toute les données de l' pour la modification
        ...
        $this->render('layout.php', 'donnees-form.php', array(
            ...
            "values" => $values // permet de récupérer toute les données de l' en cas de modification
        ));
    }
}