...
class EntityRepository
{
    ...
    public function getFields() // méthode permettant de recolter les données des champs/colonne de la table, c'est un code générique, on récuperera les données de n'importe quelle table
    {
        ...
        return array_splice($r, 1); // permet de ne pas récupérer le premier champs i dans le formulaire, dans la BDD grace à la fonction prédéfinie array_splice()
    }
    ...
    {
        //  $q = $this->getDb()->query("SELECT * FROM");
        $q = $this->getDb()->query("SELECT * FROM " . $this->table); //  requete permettant de selectionner toute une table, $this->table : représente dans notre cas la table '
        $r = $q->fetchAll(\PDO::FETCH_ASSOC); 
        return $r;
    }
